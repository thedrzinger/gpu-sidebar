...
1. On the GPU host, run the bundled reporter — no Python needed:
   ...
   ```sh
   npx -p gpu-sidebar gpu-sidebar-reporter --port 9100
   ```

   (Modern npm needs `-p <package>` when the bin name differs from the
   package name — a bare `npx gpu-sidebar-reporter` 404s, confirmed
   live. Or `npm i -g gpu-sidebar`, then `gpu-sidebar-reporter`.) The reporter
